...
module reader
   ...
   public :: readfile
   public :: molecule, ifile_xyz, ifile_wfn, ifile_grd, ifile_wfx, ifile_cube
   ...
   integer, parameter :: ifile_cube = 5
...
subroutine readcube(file, m)
! returns: molecule type, ! origine de la grille, fin de la grille, increments en pas selon x,y,z
    ...
    DOUBLE PRECISION,DIMENSION(3,3) :: X,H
	DOUBLE PRECISION,DIMENSION(3) :: V,Z,U,G,r,vec0
	REAL*8,DIMENSION(3) :: xinit,xmax,xinc
	DOUBLE PRECISION :: scal
	INTEGER, DIMENSION(:),ALLOCATABLE :: Zato
	DOUBLE PRECISION, DIMENSION(:),ALLOCATABLE :: Mato,Xxato,Yyato,Zzato
	CHARACTER(len=500) :: comm1,comm2,inpname
	INTEGER :: ci,cj,cl,ck,istat
	INTEGER, DIMENSION(3) :: com
!	REAL*8 :: wk1(3), wk2(3), hvecs(3, 3)
	DOUBLE PRECISION :: dx1,dx2,dx3
!	REAL,DIMENSION(:,:,:),ALLOCATABLE :: cubedens,redgrad,hessian
	REAL :: somme,atm,prod,start,finish
...
!==================================================================!
!!!!!!!	TABLE CONTAINS ALL THE VALUES OF THE .CUBE FILE !!!!!!!
!==================================================================!
	OPEN(lu,file=file,status='old')
		READ(lu,'(A)') comm1 
		READ(lu,'(A)') comm2 
		READ(lu,*) m%n,m%xinit0
        allocate(m%x(3,m%n),stat=istat)
        if (istat /= 0) call error('readcube', 'could not allocate memory for atomic positions', faterr)
        allocate (m%z(m%n), stat=istat)
        if (istat /= 0) call error('readcube', 'could not allocate memory for atomic numbers', faterr)
          allocate (m%ifrag(m%n), stat=istat)
      if (istat /= 0) call error('readxyz', 'could not allocate memory for fragment ids', faterr)
      allocate (m%q(m%n), stat=istat)
      if (istat /= 0) call error('readwfn', 'could not allocate memory for atomic charges', faterr)
		ALLOCATE(Mato(m%n),stat=istat)
        if (istat /= 0) call error('readcube', 'could not allocate memory for atomic masses (sad, we do not need it) :(', faterr)

	DO ci=1,3
		READ(lu,fmt=*) com(ci),X(ci,1),X(ci,2),X(ci,3)
        DO ck=1,3
            if ((ci .NE. ck).AND.(abs(X(ci,ck)) .GT. 0.00001)) then
             call error ('readcube', 'not an orthogonal grid (sad, we need it) :(', faterr)
             END IF
        end do
	END DO
   ! Easy solution, not correct
   m%xcom0(1)=com(1)
   m%xcom0(2)=com(2)
   m%xcom0(3)=com(3)

	m%xinc0(1) = sqrt(X(1,1)**2)   
	m%xinc0(2) = sqrt(X(2,2)**2)
	m%xinc0(3) = sqrt(X(3,3)**2)

        if ((m%xinc0(1) > 0.20d0) .or. (m%xinc0(1) > 0.20d0) .or. (m%xinc0(1) >0.20d0)) then
                call error('readcube', 'grid quality is low (spacing below 0.2 bohr)!',warning)
        end if 
    do ci=1,3
        m%xmax0(ci)=m%xinit0(ci)+m%xinc0(ci)*com(ci)
    end do
!=====================================================!	
!       READING THE ATOMS
!=====================================================!
	DO ck=1,m%n
		READ(unit=lu,fmt=*) m%z(ck),Mato(ck),m%x(:,ck)
	END DO
        
!================================================!
!	Informations from the matrix
!================================================!
	ALLOCATE (m%cubedens(0:com(1)-1,0:com(2)-1,0:com(3)-1))!redgrad(com(1),com(2),com(3)),hessian(com(1),com(2),com(3)))
	DO cl=0,com(1)-1
		DO ck=0,com(2)-1
			READ(unit=lu,fmt=*) (m%cubedens(cl,ck,cj),cj=0,com(3)-1)
		END DO
	END DO
