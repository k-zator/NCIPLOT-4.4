...
def calculate_energy_single(output, ispromol):
    ...
    vdw = np.array([float(i.split(":")[1].split()[0]) for i in output[sis+38:sis+45]])
    rep = np.array([float(i.split(":")[1].split()[0]) for i in output[sis+67:sis+74]])
...
def calculate_energy_cluster(output, ispromol, mol1, mol2, filename):
    ...
    # create a mask for the order of no_clusters to switch to the other equation for the flagged complexes
    print(CP_Atoms)
    print(sigma_bonds)
    E_sum = []; E_polar = []; E_vdw = []; polar_i = 0; nonpolar_i = 0
    for _ in range(no_clusters):
        ...
        E_vdw.append(e_vdw)
    print("SUMMED INDICES FOR CHECK:")
    print("Polar n=1      :  ", polar_i)
    print("Non-Polar n=1  :  ", nonpolar_i)
