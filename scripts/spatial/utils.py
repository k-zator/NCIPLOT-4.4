...
def readcube(filename, verbose=True):
    ...
    np.savetxt("pts_all.dat", pts.reshape((npx*npy*npz, 3)))
    #print(pts.shape)
    #print(np.unique(np.where(np.absolute(pts - 0.336126)< 1e-5)[0]))
    #print(np.unique(np.where(np.absolute(pts + 1.586795)< 1e-5)[1]))
    #print(np.unique(np.where(np.absolute(pts - 3.340939)< 1e-5)[2]))
    #print(pts[38, 25, 31])
    #print(carray[38, 25, 31])
...
def writecube(filename, cl, X_iso, labels, header, verbose=True):
    ...
    if o1 < extr_x[0]-delta*incrx:
        delta_x[0]= delta*incrx
        o1 = extr_x[0] - delta_x[0]
    if extr_x[1]+delta*incrx < e1:
        delta_x[1]= delta*incrx

    e2 = o2 + (npy-1) * incry
    if o2 < extr_y[0]-delta*incry:
        delta_y[0]= delta*incry
        o2 = extr_y[0] - delta_y[0]
    if extr_y[1]+delta*incry < e2:
        delta_y[1]= delta*incry
        
    e3 = o3 + (npz-1) * incrz
    if o3 < extr_z[0]-delta*incrz:
        delta_z[0]= delta*incrz
...
def pos_dens_grad_matrix(filename, save_to_csv=False):
    ...
    """
    densheader, denspts, densarray, densatcoords = readcube(filename + "-dens.cube")
    gradheader, gradpts, gradarray, gradatcoords = readcube(filename + "-grad.cube")
    ...
    if not np.allclose(densatcoords, gradatcoords):
        raise ValueError("Atomic coordinates in file do not match!")
    ...
    return densheader, final.reshape(densarray.size, 5), densatcoords
